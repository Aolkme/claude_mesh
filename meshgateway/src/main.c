...
/* ─── 心跳发送回调 ─── */
static serial_port_t *g_serial_for_hb = NULL;

static void heartbeat_send(void *userdata) {
    (void)userdata;
    if (!g_serial_for_hb) return;

    uint8_t proto_buf[64];
    size_t proto_len = 0;
    if (proto_build_heartbeat(proto_buf, sizeof(proto_buf), &proto_len) != MESH_OK) return;

    uint8_t frame[68];
    frame[0] = 0x94; frame[1] = 0xC3;
    frame[2] = (uint8_t)(proto_len >> 8);
    frame[3] = (uint8_t)(proto_len & 0xFF);
    memcpy(frame + 4, proto_buf, proto_len);

    serial_port_write(g_serial_for_hb, frame, 4 + proto_len);
    LOG_INFO("main", "Heartbeat sent");
}

/* ─── gateway_state 发送回调 ─── */
static int gstate_send_raw(const uint8_t *buf, size_t len, void *ctx) {
    serial_port_t *sp = (serial_port_t *)ctx;
    return serial_port_write(sp, buf, len);
}

/* ─── want_config 握手：请求设备推送节点列表 ─── */
static void send_want_config(serial_port_t *sp) {
    uint8_t proto_buf[64];
    size_t  proto_len = 0;
    uint32_t config_id = (uint32_t)time(NULL); /* 随机 config_id */
    ...
    frame[3] = (uint8_t)(proto_len & 0xFF);
    memcpy(frame + 4, proto_buf, proto_len);
    serial_port_write(sp, frame, 4 + proto_len);
...
int main(int argc, char *argv[]) {
    ...
    /* 命令行参数覆盖配置文件 */
    if (serial_device) strncpy(cfg.serial_device, serial_device, sizeof(cfg.serial_device) - 1);
    ...
    if (!cfg.serial_device[0]) {
        LOG_ERROR("main", "No serial device configured. Use -p or set serial.device in config.");
        log_close();
        return 1;
    }
    ...
    if (!sp) {
        LOG_ERROR("main", "Cannot open serial port: %s", cfg.serial_device);
        log_close();
        return 1;
    }
    LOG_INFO("main", "Serial port %s opened at %u baud", cfg.serial_device, cfg.serial_baudrate);
    g_serial_for_hb = sp;

    /* 初始化心跳 */
    heartbeat_init((uint64_t)cfg.heartbeat_interval_s * 1000, heartbeat_send, NULL);
    ...
    gstate.send_raw   = gstate_send_raw;
    gstate.send_ctx   = sp;
